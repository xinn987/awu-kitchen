...
邀请通过小程序分享卡片传递，单次使用、24 小时有效。首版不提供手输邀请码和永久链接。
...
2. 小程序分享 `/pages/onboarding/index?invite=<token>`。
3. 接收者先预览家庭名称，再填写家庭称谓并确认。
4. 云函数在事务中消费邀请、创建成员并写入用户的 `activeMemberId`。

重复打开已使用或过期链接只显示明确错误，不建立成员关系。
